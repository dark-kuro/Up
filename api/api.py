...
class BaseApi(object):
    ...
    def auth(self, client_info, scope=[PHOTOS]):
        self.store = Storage(self.join('.user_auth.json'))
        credentials = self.store.get()
        if not credentials:
            credentials = tools.run_flow(client.flow_from_clientsecrets(client_info, scope=scope), self.store)
    ...
    def __init__(self, client_info=None):
        api = self.auth(self.join('client_info.json') if not client_info else client_info)
